...
public class Hero {
    ...
    public Hero(String name, int hp, int mana, int attackPower,
                int defense, int gold, Inventory inventory) {
        this.name       = name;
        this.hp         = hp;
        this.maxHp      = hp;
        this.mana       = mana;
    ...
    /**
     * Creates a memento capturing the full mutable state.
     * Only Hero can construct HeroMemento (package-private constructor).
     */
    public HeroMemento createMemento() {
        return new HeroMemento(
                name, hp, mana, gold, maxHp, attackPower, defense,
                inventory.getArtifacts()
        );
    }

    /**
     * Restores hero state from a memento.
     * Only Hero can read HeroMemento internals (package-private getters).
     */
